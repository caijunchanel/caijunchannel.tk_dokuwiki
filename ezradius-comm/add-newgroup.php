...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	...
	if ($_POST['price']!='') {
		...
		$record['package_id']=$groupname;
		$record['price']=$_POST['price'];
		$db->add($record);
		$db->close;
	}
	
	//insert initial attribute for package
	if ($userPassword!='MD5-Password') {
		//insert Auth-Type
		$insertSQL= sprintf("INSERT INTO radgroupcheck (GroupName, Attribute, op, `Value`) VALUES (%s, %s, %s, %s); \n",
                       GetSQLValueString($_POST['GroupName'], "text"),
                       GetSQLValueString("Auth-Type", "text"),
	...
	$insertSQL = sprintf ("INSERT INTO radgroupreply (GroupName, Attribute, op, `Value`) VALUES (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['GroupName'], "text"),
                       GetSQLValueString("Acct-Interim-Interval", "text"),
                       GetSQLValueString(":=", "text"),
                       GetSQLValueString("60", "text"));
	mysql_select_db($database_cnRadius, $cnRadius);
  	$Result1 = mysql_query($insertSQL, $cnRadius) or die(mysql_error());
	...
	if (isset($_POST["idle"])&&($_POST["idle"]!='')) {
		...
		$insertSQL = sprintf ("INSERT INTO radgroupreply (GroupName, Attribute, op, `Value`) VALUES  (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['GroupName'], "text"),
                       GetSQLValueString("Idle-Timeout", "text"),
                       GetSQLValueString(":=", "text"),
                       GetSQLValueString($_POST['idle'], "text"));
					   
		mysql_select_db($database_cnRadius, $cnRadius);
  		$Result1 = mysql_query($insertSQL, $cnRadius) or die(mysql_error());
	}
	
	if (isset($_POST["simultan"])&&($_POST["simultan"]!='')) {
		//simultaneous use
		$insertSQL = sprintf ("INSERT INTO radgroupcheck (GroupName, Attribute, op, `Value`) VALUES  (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['GroupName'], "text"),
                       GetSQLValueString("Simultaneous-Use", "text"),
                       GetSQLValueString(":=", "text"),
                       GetSQLValueString($_POST['simultan'], "text"));
	...
	if (isset($_POST["download_limit"])&&($_POST["download_limit"]!='')) {
		//download limit
		switch ($_POST["download_limit_select"]) {
			case "all":
		...
		if ($attr!="none")
			$insertSQL = sprintf ("INSERT INTO radgroupcheck (GroupName, Attribute, op, `Value`) VALUES  (%s, %s, %s, %s)",
                       ...
                       GetSQLValueString($attr, "text"),
                       GetSQLValueString(":=", "text"),
                       GetSQLValueString($_POST['download_limit'], "text"));
	
		mysql_select_db($database_cnRadius, $cnRadius);
  		$Result1 = mysql_query($insertSQL, $cnRadius) or die(mysql_error());
	}
	
	if (isset($_POST["download_speed_limit"])&&($_POST["download_speed_limit"]!='')) {
		//download speed limit
		$insertSQL = sprintf ("INSERT INTO radgroupreply (GroupName, Attribute, op, `Value`) VALUES  (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['GroupName'], "text"),
                       GetSQLValueString("WISPr-Bandwidth-Max-Down", "text"),
                       GetSQLValueString(":=", "text"),
                       GetSQLValueString($_POST['download_speed_limit'], "text"));
	
		mysql_select_db($database_cnRadius, $cnRadius);
  		$Result1 = mysql_query($insertSQL, $cnRadius) or die(mysql_error());
	}
	
	if (isset($_POST["upload_speed_limit"])&&($_POST["upload_speed_limit"]!='')) {
		//download speed limit
		$insertSQL = sprintf ("INSERT INTO radgroupreply (GroupName, Attribute, op, `Value`) VALUES  (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['GroupName'], "text"),
                       GetSQLValueString("WISPr-Bandwidth-Max-Up", "text"),
                       GetSQLValueString(":=", "text"),
                       GetSQLValueString($_POST['upload_speed_limit'], "text"));
	
		mysql_select_db($database_cnRadius, $cnRadius);
  		$Result1 = mysql_query($insertSQL, $cnRadius) or die(mysql_error());
	}
	
	if (isset($_POST["expired"])&&($_POST["expired"]!='')) {
		//expiration
		$insertSQL = sprintf ("INSERT INTO radgroupcheck (GroupName, Attribute, op, `Value`) VALUES  (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['GroupName'], "text"),
                       GetSQLValueString("Expiration", "text"),
                       GetSQLValueString(":=", "text"),
                       GetSQLValueString($_POST['expired'], "text"));
  ...
  $insertGoTo = "group-detail.php?groupname=" . $_POST['GroupName'] . "";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<?php
	if (!$valid) {
		echo "<center><b><font color='red'>Package name must be filled</font></b></center>";
	}
?>
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
  <table align="center">
    <tr valign="baseline">
      ...
      <td><input type="text" name="GroupName" value="" size="32" /> 
	  <?php
	  if ($currShow==1) {
	  ?>
      Price 
    ...
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Idle Timeout </td>
      <td><input name="idle" type="text" id="idle" value="300" size="5" />
        seconds (min. 60) </td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Simultanous Use </td>
      <td><input name="simultan" type="text" id="simultan" value="1" size="5" />
        time(s)</td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Time limit </td>
      <td><input name="time" type="text" id="time" size="15" />
        seconds
    ...
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Download/Upload limit </td>
      <td><input name="download_limit" type="text" id="download_limit" />
        bytes
          <select name="download_limit_select" id="download_limit_select">
            <option value="none">None</option>
    ...
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Download speed limit </td>
      <td><input name="download_speed_limit" type="text" id="download_speed_limit" />
        b/s</td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Upload speed limit </td>
      <td><input name="upload_speed_limit" type="text" id="upload_speed_limit" />
        b/s</td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Expired on </td>
      <td><input name="expired" type="text" id="expired" />
        <img src="images/cal.gif" alt="Date" width="16" height="16" onclick="MM_callJS('NewCal(\'expired\',\'ddmmmyyyy\')')" /></td>
    </tr>
    <tr valign="baseline">
