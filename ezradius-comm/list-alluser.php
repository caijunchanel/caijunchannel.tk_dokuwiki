...
if (isset($_GET['btnCari']) || ($_GET['srcUname']<>'')) {
	$query_rsAllUser = "SELECT distinct(radcheck.username),radcheck.value, radcheck.id,usergroup.groupname as groupname FROM radcheck LEFT JOIN usergroup ON radcheck.username=usergroup.username WHERE Attribute LIKE '%-Password' AND radcheck.username LIKE '%".$_GET['srcUname']."%' GROUP BY UserName ORDER BY id";

} else {
	$query_rsAllUser = "SELECT distinct(radcheck.username),radcheck.value, radcheck.id,usergroup.groupname as groupname FROM radcheck LEFT JOIN usergroup ON radcheck.username=usergroup.username WHERE Attribute LIKE '%-Password' GROUP BY UserName ORDER BY id";
}
$query_limit_rsAllUser = sprintf("%s LIMIT %d, %d", $query_rsAllUser, $startRow_rsAllUser, $maxRows_rsAllUser);
$rsAllUser = mysql_query($query_limit_rsAllUser, $cnRadius) or die(mysql_error());
...
<form id="form1" name="form1" method="get" action="list-alluser.php">
  <input type="text" name="srcUname" id="srcUname" />
<input type="submit" name="btnCari" id="btnCari" value="Go" />
</form>
</fieldset>
<p>Record <?php echo ($startRow_rsAllUser + 1) ?> &nbsp;to <?php echo min($startRow_rsAllUser + $maxRows_rsAllUser, $totalRows_rsAllUser) ?> &nbsp;from <?php echo $totalRows_rsAllUser ?> &nbsp;total</p>
<p><a href="<?php printf("%s?pageNum_rsAllUser=%d%s&srcUname=%s", $currentPage, 0, $queryString_rsAllUser, $_GET['srcUname']); ?>"><img src="images/first.png" alt="First" width="60" height="15" border="0" /></a> <a href="<?php printf("%s?pageNum_rsAllUser=%d%s&srcUname=%s", $currentPage, max(0, $pageNum_rsAllUser - 1), $queryString_rsAllUser, $_GET['srcUname']); ?>"><img src="images/prev.png" alt="Prev." width="60" height="15" border="0" /></a> <a href="<?php printf("%s?pageNum_rsAllUser=%d%s&srcUname=%s", $currentPage, min($totalPages_rsAllUser, $pageNum_rsAllUser + 1), $queryString_rsAllUser, $_GET['srcUname']); ?>"><img src="images/next.png" alt="Next" width="60" height="15" border="0" /></a> <a href="<?php printf("%s?pageNum_rsAllUser=%d%s&srcUname=%s", $currentPage, $totalPages_rsAllUser, $queryString_rsAllUser, $_GET['srcUname']); ?>"><img src="images/last.png" alt="Last" width="60" height="15" border="0" /></a></p>
...
<thead class="mythead">
  ...
  <?php do { ?>
    <tr>
      <th><?php echo $row_rsAllUser['id']; ?></th>
      <th><a href="user-detail.php?username=<?php echo $row_rsAllUser['username']; ?>"><?php echo $row_rsAllUser['username']; ?></a></th>
      <td><?php echo $row_rsAllUser['value']; ?></td>
      <td><a href="group-detail.php?groupname=<?php echo $row_rsAllUser['groupname']; ?>"><?php echo $row_rsAllUser['groupname']; ?></a></td>
      <td><?php
	  	//find if the user is online
	  	mysql_select_db($database_cnRadius, $cnRadius);
		$query_online = sprintf("SELECT UserName, AcctStopTime FROM radacct WHERE UserName = %s AND AcctStopTime is NULL", GetSQLValueString($row_rsAllUser['username'], "text"));
