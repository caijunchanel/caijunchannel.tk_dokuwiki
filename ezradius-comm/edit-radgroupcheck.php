...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE radgroupcheck SET GroupName=%s, Attribute=%s, op=%s, `Value`=%s WHERE id=%s",
  ...
  $updateGoTo = "group-detail.php?groupname=" . $_POST['GroupName'] . "";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_rsRadgroupcheck = "-1";
if (isset($_GET['attrID'])) {
  $colname_rsRadgroupcheck = $_GET['attrID'];
}
mysql_select_db($database_cnRadius, $cnRadius);
$query_rsRadgroupcheck = sprintf("SELECT * FROM radgroupcheck WHERE id = %s", GetSQLValueString($colname_rsRadgroupcheck, "int"));
$rsRadgroupcheck = mysql_query($query_rsRadgroupcheck, $cnRadius) or die(mysql_error());
$row_rsRadgroupcheck = mysql_fetch_assoc($rsRadgroupcheck);
$totalRows_rsRadgroupcheck = mysql_num_rows($rsRadgroupcheck);
...
<title>Edit radgroupcheck</title>
<!-- InstanceEndEditable -->
...
<fieldset>
<legend>Edit radgroupcheck</legend>
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
  <table align="center">
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">PackageName:</td>
      <td><?php echo htmlentities($row_rsRadgroupcheck['GroupName'], ENT_COMPAT, 'utf-8'); ?></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Attribute:</td>
      <td><?php echo htmlentities($row_rsRadgroupcheck['Attribute'], ENT_COMPAT, 'utf-8'); ?></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Op:</td>
      <td><input type="text" name="op" value="<?php echo htmlentities($row_rsRadgroupcheck['op'], ENT_COMPAT, 'utf-8'); ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Value:</td>
      <td><input type="text" name="Value" value="<?php echo htmlentities($row_rsRadgroupcheck['Value'], ENT_COMPAT, 'utf-8'); ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
  ...
  <input type="hidden" name="id" value="<?php echo $row_rsRadgroupcheck['id']; ?>" />
  <input name="GroupName" type="hidden" id="GroupName" value="<?php echo $row_rsRadgroupcheck['GroupName']; ?>" />
  <input name="Attribute" type="hidden" id="Attribute" value="<?php echo $row_rsRadgroupcheck['Attribute']; ?>" />
</form>
</fieldset>
<!-- InstanceEndEditable -->
    <!-- end #mainContent -->
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsRadgroupcheck);
?>
