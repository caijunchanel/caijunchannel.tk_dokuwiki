...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE nas SET nasname=%s, shortname=%s, type=%s, ports=%s, secret=%s, community=%s, `description`=%s WHERE id=%s",
                       GetSQLValueString($_POST['nasname'], "text"),
                       GetSQLValueString($_POST['shortname'], "text"),
                       GetSQLValueString($_POST['type'], "text"),
                       GetSQLValueString($_POST['ports'], "int"),
...
$colname_rsEditNAS = "-1";
if (isset($_GET['id'])) {
  $colname_rsEditNAS = (get_magic_quotes_gpc()) ? $_GET['id'] : addslashes($_GET['id']);
}
mysql_select_db($database_cnRadius, $cnRadius);
$query_rsEditNAS = sprintf("SELECT id, nasname, shortname, type, ports, secret, community, `description` FROM nas WHERE id = %s", GetSQLValueString($colname_rsEditNAS, "int"));
$rsEditNAS = mysql_query($query_rsEditNAS, $cnRadius) or die(mysql_error());
$row_rsEditNAS = mysql_fetch_assoc($rsEditNAS);
$totalRows_rsEditNAS = mysql_num_rows($rsEditNAS);
...
<div id="container">
  ...
  <div id="mainContent">
	...
	<fieldset><label>Edit NAS</label>
    <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
      <table align="center">
        <tr valign="baseline">
          <td nowrap align="right">Id:</td>
          <td><?php echo $row_rsEditNAS['id']; ?></td>
        </tr>
        <tr valign="baseline">
          <td nowrap align="right">Nasname:</td>
          <td><input type="text" name="nasname" value="<?php echo $row_rsEditNAS['nasname']; ?>" size="32"></td>
        </tr>
        <tr valign="baseline">
          <td nowrap align="right">Shortname:</td>
          <td><input type="text" name="shortname" value="<?php echo $row_rsEditNAS['shortname']; ?>" size="32"></td>
        </tr>
        <tr valign="baseline">
          <td nowrap align="right">Type:</td>
          <td><input type="text" name="type" value="<?php echo $row_rsEditNAS['type']; ?>" size="32"></td>
        </tr>
        <tr valign="baseline">
          <td nowrap align="right">Ports:</td>
          <td><input type="text" name="ports" value="<?php echo $row_rsEditNAS['ports']; ?>" size="32"></td>
        </tr>
        <tr valign="baseline">
          <td nowrap align="right">Secret:</td>
          <td><input type="text" name="secret" value="<?php echo $row_rsEditNAS['secret']; ?>" size="32"></td>
        </tr>
        <tr valign="baseline">
          <td nowrap align="right">Community:</td>
          <td><input type="text" name="community" value="<?php echo $row_rsEditNAS['community']; ?>" size="32"></td>
        </tr>
        <tr valign="baseline">
          <td nowrap align="right">Description:</td>
          <td><input type="text" name="description" value="<?php echo $row_rsEditNAS['description']; ?>" size="32"></td>
        </tr>
        <tr valign="baseline">
      ...
      <input type="hidden" name="id" value="<?php echo $row_rsEditNAS['id']; ?>">
    </form>
	</fieldset>
    <p>&nbsp;</p>
    <!-- InstanceEndEditable -->
