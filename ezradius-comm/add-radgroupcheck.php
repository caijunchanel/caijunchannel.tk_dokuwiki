...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	...
	$query_rsRadGroupCheck= sprintf("SELECT * FROM radgroupcheck WHERE GroupName = %s AND Attribute=%s", GetSQLValueString($_POST['GroupName'], "text"), GetSQLValueString($_POST['Attribute'], "text"));
	...
	if ($totalRows_rsRadGroupCheck<=0) { 
		...
		$insertSQL = sprintf("INSERT INTO radgroupcheck (GroupName, Attribute, op, `Value`) VALUES (%s, %s, %s, %s)",
  		...
  		$insertGoTo = "group-detail.php?groupname=" . $_GET['groupname'] . "";
  		if (isset($_SERVER['QUERY_STRING'])) {
...
<fieldset>
<legend>Add new groupcheck attribute</legend>
<?php
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
  <table align="center">
    <tr valign="baseline">
      ...
      <td><input type="text" name="GroupName" value="<?php echo $_GET['groupname']; ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Attribute:</td>
      <td>
	  <?php include ('static-authcheck.php'); ?>
      </td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Op:</td>
      <td>
	  <?php include('static-operator.php'); ?>
      </td>
    </tr>
    <tr valign="baseline">
...
<div id="CollapsiblePanel1" class="CollapsiblePanel">
  ...
  require(basename("static-available-authcheck.php"));
?></div>
</div>
