...
/*
$footer_text = '<center>
                  <a href="http://www.example.de/register.php">[register]</a> 
                  <a href="http://www.example.de/rules.php">[terms and conditions]</a>  
                  <a href="http://www.example.de/for_free.php">[do not register]</a> 
                </center>';
*/
$footer_text = '<center>Labkom HotSpot Login</center>';
...
if ($_GET['login'] == login) {
  ...
  $titel = 'Logging in to Labkom HotSpot'; 
  $headline = 'Logging in to Labkom HotSpot';
  $bodytext = '';
  
  # username cookie -- added by achmad
  $name = $_GET['UserName'];
  $expire = time() + 315360000;
  setcookie('username', $name, $expire, '/', $_SERVER['HTTP_HOST'], true);
  #added 
...
if ($_GET['res'] == success) {
 ...
 // $body_onload = 'onLoad="javascript:popUp(' . $loginpath . '?res=popup&uamip=' . $_GET['uamip'] . '&uamport=' . $_GET['uamport'] . '&timeleft='  . $_GET['timeleft'] . ')"';
  ...
  # added by achmad
  print "<center><iframe src='front-status.php' width='200px' height='175px' frameborder='0' /></center>";
...
if ($_GET['res'] == failed) {
  ...
  $result = 2;
  $titel = 'Labkom HotSpot Login Failed';
  $headline = 'Labkom HotSpot Login Failed';
...
if ($_GET['res'] == logoff) {
  ...
  $result = 3;
  $titel = 'Logged out from Labkom HotSpot';
  $headline = 'Logged out from Labkom HotSpot';
  ...
  <html>
    ...
    function popUp(URL) {
      if (self.name != \"chillispot_popup\") {
        chillispot_popup = window.open(URL, 'chillispot_popup', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,width=350,height=505');
      }
    }
    ...
    function doOnLoad(result, URL, userurl, redirurl, timeleft) {
      ...
      if ((result == 2) && (self.name != \"chillispot_popup\")) {
        //chillispot_popup = window.open('', 'chillispot_popup', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,width=400,height=410');
