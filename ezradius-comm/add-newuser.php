...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	...
	if ($_POST["chkCleartext"]=="checked") {
		if ($freeradiusVersion=="1.1.3") {
			$insertSQL = sprintf("INSERT INTO radcheck (UserName, Attribute, op, `Value`) VALUES (%s, %s, %s, %s)",
  	...
  	$insertSQL = sprintf("INSERT INTO radcheck (UserName, Attribute, op, `Value`) VALUES (%s, %s, %s, md5(%s))",
                       GetSQLValueString($_POST['username'], "text"),
                       GetSQLValueString($_POST['Attribute'], "text"),
                       GetSQLValueString($_POST['op'], "text"),
                       GetSQLValueString($_POST['Value'], "text"));
  ...
  //check if attach to group
  if ($_POST["selGroupList"]!="none") {
  	//attach user to specified group
	$insertSQL = sprintf("INSERT INTO usergroup (UserName, GroupName, priority) VALUES (%s, %s, %s)",
                       GetSQLValueString($_POST['username'], "text"),
                       GetSQLValueString($_POST['selGroupList'], "text"),
  ...
  $insertGoTo = "user-detail.php?username=" . $_POST['username'] . "";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_cnRadius, $cnRadius);
$query_Recordset1rsAllGroups = "SELECT distinct(groupname) FROM radgroupcheck";
$Recordset1rsAllGroups = mysql_query($query_Recordset1rsAllGroups, $cnRadius) or die(mysql_error());
$row_Recordset1rsAllGroups = mysql_fetch_assoc($Recordset1rsAllGroups);
$totalRows_Recordset1rsAllGroups = mysql_num_rows($Recordset1rsAllGroups);
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
  <table align="center">
    ...
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Attach to package </td>
      <td><select name="selGroupList" id="selGroupList">
        <option value="none">None</option>
        <?php
do {  
?>
        <option value="<?php echo $row_Recordset1rsAllGroups['groupname']?>">
		<?php
		$db = new FFDB();
		$db->open('price');
		$rec=$db->getbykey($row_Recordset1rsAllGroups['groupname']);
		$db->close();
		?>
		<?php
		if ($currShow==1) { 
			echo $row_Recordset1rsAllGroups['groupname']."(".$currSymbol." ".$rec['price'].")";
		} else {
			echo $row_Recordset1rsAllGroups['groupname'];
		}
		?>
		</option>
        <?php
} while ($row_Recordset1rsAllGroups = mysql_fetch_assoc($Recordset1rsAllGroups));
  $rows = mysql_num_rows($Recordset1rsAllGroups);
  if($rows > 0) {
      mysql_data_seek($Recordset1rsAllGroups, 0);
	  $row_Recordset1rsAllGroups = mysql_fetch_assoc($Recordset1rsAllGroups);
  }
?>
      </select>
  ...
  <input type="hidden" name="Attribute" value="<?php echo $userPassword; ?>" />
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($Recordset1rsAllGroups);
?>
