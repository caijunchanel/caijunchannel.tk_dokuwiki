...
mysql_select_db($database_cnRadius, $cnRadius);
$query_rsGroupname = "SELECT distinct(Groupname) FROM radgroupcheck";
$rsGroupname = mysql_query($query_rsGroupname, $cnRadius) or die(mysql_error());
...
<fieldset>
<legend>Add user <?php echo $_GET['username']; ?> to package:</legend>
...
do {  
?>
        <option value="<?php echo $row_rsGroupname['Groupname']?>" >
		<?php
		$db = new FFDB();
		$db->open('price');
		$rec=$db->getbykey($row_rsGroupname['Groupname']);
		$db->close();
		?>
		<?php
		if ($currShow==1) { 
			echo $row_rsGroupname['Groupname']."(".$currSymbol." ".$rec['price'].")";
		} else {
			echo $row_rsGroupname['Groupname'];
		}
		?>
		</option>
        <?php
} while ($row_rsGroupname = mysql_fetch_assoc($rsGroupname));
?>
      </select>
      </td>
    </tr>
    ...
    <tr valign="baseline">
      ...
      <td><input type="submit" value="Attach to package" /></td>
    </tr>
  </table>
