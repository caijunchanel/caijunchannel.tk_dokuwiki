...
<?php
if ((isset($_POST['btnUpdate'])) && (isset($_POST['newgroupname'])) && ($_POST['newgroupname']<>'')) {
	$updateSQL="update usergroup set groupname='".$_POST['newgroupname']."' where username='".$_POST['username']."'";
	
	if ($_POST['newgroupname']=="none") {
		//revoke user from a group
		$updateSQL="delete from usergroup where username='".$_POST['username']."'";
	}
	
	mysql_select_db($database_cnRadius, $cnRadius);
  	$Result1 = mysql_query($updateSQL, $cnRadius) or die(mysql_error());
	
	$updateGoTo = "user-detail.php?username=" . $_POST['username'] . "";
...
mysql_select_db($database_cnRadius, $cnRadius);
$query_rsAllGroup = "SELECT distinct ( `GroupName` ) FROM `radgroupcheck`";
$rsAllGroup = mysql_query($query_rsAllGroup, $cnRadius) or die(mysql_error());
$row_rsAllGroup = mysql_fetch_assoc($rsAllGroup);
$totalRows_rsAllGroup = mysql_num_rows($rsAllGroup);
?>
...
<fieldset>
<legend>Edir User-Package Mapping</legend>
<form name="form1" method="post" action="edit-user-to-group.php">
  <table width="100%" border="0">
    <tr>
      <td width="16%"><strong>Old Package </strong></td>
      <td width="2%">:</td>
      <td width="82%"><?php echo $row_rsUserGroup['GroupName']; ?></td>
    </tr>
    <tr>
      <td><strong>New Package </strong></td>
      <td>:</td>
      <td><select name="newgroupname" id="newgroupname">
          <option value="none" <?php if (!(strcmp("none", $row_rsAllGroup['GroupName']))) {echo "selected=\"selected\"";} ?>>None</option>
          <?php
do {  
?>
          <option value="<?php echo $row_rsAllGroup['GroupName']?>"<?php if (!(strcmp($row_rsAllGroup['GroupName'], $row_rsAllGroup['GroupName']))) {echo "selected=\"selected\"";} ?>>
		  <?php
		$db = new FFDB();
		$db->open('price');
		$rec=$db->getbykey($row_rsAllGroup['GroupName']);
		$db->close();
		?>
		<?php
		if ($currShow==1) { 
			echo $row_rsAllGroup['GroupName']."(".$currSymbol." ".$rec['price'].")";
		} else {
			echo $row_rsAllGroup['GroupName'];
		}
		?>
		  </option>
          <?php
} while ($row_rsAllGroup = mysql_fetch_assoc($rsAllGroup));
  $rows = mysql_num_rows($rsAllGroup);
  if($rows > 0) {
      mysql_data_seek($rsAllGroup, 0);
	  $row_rsAllGroup = mysql_fetch_assoc($rsAllGroup);
  }
?>
      </select></td>
